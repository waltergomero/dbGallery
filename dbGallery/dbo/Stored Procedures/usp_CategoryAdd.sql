...
BEGIN
	...
	IF EXISTS(SELECT TOP 1 * FROM [Category] WITH(NOLOCK) WHERE CategoryName = @p_chrCategoryName)
		BEGIN
    ...
    BEGIN TRY
		...
		SELECT @p_intStatusId = StatusId FROM Status WHERE StatusName = 'Active';
            INSERT INTO Category(CategoryName, Description, CategoryImage, ParentCategoryId, StatusId) 
